...
export default function SurveyForm({
  ...
  async function handleSubmit() {
      ...
      dim.questions.forEach(q => {
        ...
        if (type === 'text') {
          ...
          if (textVal && textVal.trim()) {
            responsesArray.push({
              questionId: q.id,
              value: null,  // NULL para texto (no contribuye al score)
    ...
    // Guardar todas las respuestas en un solo batch
    if (responsesArray.length > 0) {
      const dtos = responsesArray.map(r => ({ questionId: r.questionId, value: r.value, textValue: r.textValue }))
      const upsertResult = await responseRepo.upsertBatch(respondentId!, dtos)
